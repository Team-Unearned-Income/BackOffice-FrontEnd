    ...
    <template #filter-section>
      ...
      <div class="row q-py-md justify-end">
        <div style="display: flex; gap: 1rem">
          ...
          <q-btn label="등록" style="background-color: #169bd5; color: white;" @click="goRegist" />
        </div>
      </div>
      <div class="q-py-md" />
    </template>
...
const onRequest = async (_props) => {
  ...
  const tableRowResponse = await facilitiesApi.restroomList({
...
const goRegist = () => {
  router.push({ name: 'FacilitiesRestroomDetail' })
}

const goDetail = (event, target) => {
  router.push({ name: 'FacilitiesRestroomDetail', query: { idx: target.pkSeq } })
}
</script>
<style>
tbody:empty::before {
