<template>
  <div class="q-pa-md">
    <div class="form">
      ...
      <div class="row">
        <div class="col-1 form-th required">홈 메인비주얼 이미지<br />(1100x1300)</div>
        <div class="col-11 form-td">
          <ImageInput
    ...
    <div class="row justify-between q-pt-lg">
      <div style="display: flex; gap: 1rem">
        <q-btn label="목록" @click="router.push(previousQueryUrl ?? { name: 'MainVisualsList' })" />
...
const onRequest = async () => {
  ...
  const mainVisualResponse = await sitemanagementApi
    .getMainVisualInfoGet(bannerSeq.value)
    .then((res) => res.data)
  ...
  try {
    ...
    if (bannerSeq.value) {
      regResponse = await sitemanagementApi
        .updateMainVisual(bannerSeq.value, formData)
        .then((res) => res.data)
    } else {
      regResponse = await sitemanagementApi.regMainVisual(formData).then((res) => res.data)
    }

    showToast.value = true

    setTimeout(() => {
      router.replace(previousQueryUrl.value ?? { name: 'MainVisualsList' })
...
const onDelete = async () => {
  ...
  if (isDelete) {
    await sitemanagementApi.deleteMainVisual(bannerSeq.value)
    alert('성공적으로 삭제되었습니다.')
    router.replace(previousQueryUrl.value ?? { name: 'MainVisualsList' })
  }
}
