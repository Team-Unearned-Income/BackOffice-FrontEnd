<template>
  <div class="q-pa-md">
    <div class="form">
      ...
      <div class="row">
        ...
        <div class="col-11 form-td">
          <q-input class="full-width" v-model="dataModel.title" outlined center dense />
        </div>
      </div>
      <div class="row">
        <div class="col-1 form-th required">팝업 이미지<br />(1200x900)</div>
        <div class="col-11 form-td">
          <ImageInput
    ...
    <div class="row justify-between q-pt-lg">
      <div style="display: flex; gap: 1rem">
        <q-btn label="목록" @click="router.push(previousQueryUrl ?? { name: 'PopupsList' })" />
        <q-btn label="삭제" @click="onDelete" :disable="popupSeq === null" />
      </div>
      <div>
  ...
  <RegistToast
    v-if="showToast"
    :message="popupSeq ? '수정이 완료되었습니다!' : '등록이 완료되었습니다!'"
    :duration="3000"
    @hidden="
      () => {
        showToast = false
...
import { fileApi, sitemanagementApi } from '@/service/api'
...
onBeforeMount(async () => {
  popupSeq.value = new URLSearchParams(window.location.search).get('popupSeq')
})
...
const filterSetting = async () => {
  const filterList = await sitemanagementApi.getMainVisualCodeList().then((res) => res.data)
  filterOptions.value = FilterUtils.parseFilterArray(filterList)
}

const handleImageUpdate = (image) => {
  if (!image) {
    dataModel.value.fileName = ''
    dataModel.value.fileSize = 0
    dataModel.value.fileUrl = ''
    return
  }
...
const onRequest = async () => {
  ...
  const mainVisualResponse = await sitemanagementApi
    .getPopupInfoGet(popupSeq.value)
    .then((res) => res.data)
...
const onRegist = async () => {
  if ((!imageRef.value && !popupSeq.value) || !dataModel.value.fileName) {
  ...
  try {
    ...
    if (popupSeq.value)
      regResponse = await sitemanagementApi
        .updatePopup(popupSeq.value, formData)
        .then((res) => res.data)
    else regResponse = await sitemanagementApi.regPopup(formData).then((res) => res.data)

    showToast.value = true

    setTimeout(() => {
      router.replace(previousQueryUrl.value ?? { name: 'PopupsList' })
...
const onDelete = async () => {
  ...
  if (isDelete) {
    await sitemanagementApi.deletePopup(popupSeq.value)
    alert('성공적으로 삭제되었습니다.')
    router.replace(previousQueryUrl.value ?? { name: 'PopupsList' })
  }
}
