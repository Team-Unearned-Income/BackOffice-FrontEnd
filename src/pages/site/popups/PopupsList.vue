    ...
    <template #filter-section>
      ...
      <div class="row justify-between q-py-md">
        ...
        <div style="display: flex; gap: 1rem">
          <q-btn label="팝업순서관리" @click="showModal = true" />
...
const onRequest = async (_props) => {
  ...
  const tableRowResponse = await sitemanagementApi.getMainPopupList({
...
const goRegist = () => {
  router.push({ name: 'PopupsDetail' })
}

const goDetail = (event, row) => {
  event.preventDefault()

  router.push({ name: 'PopupsDetail', query: { popupSeq: row.popupSeq } })
}
</script>
<style>
tbody:empty::before {
