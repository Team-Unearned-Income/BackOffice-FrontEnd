<template>
    ...
    <template #filter-section>
      <div class="form">
        <div class="row">
          ...
          <div class="col-1 form-th">위치</div>
          <div class="col-4 form-td">
            <q-select
              v-model="tableModel.filter.inputLocationType"
  ...
  <BannerOrderEditModal
    ...
    type="BANNER"
    :tab="filterOptions.locationTypeList.slice(1)"
    title="배너순서관리"
...
const tableModel = ref({
  ...
  header: [
    ...
    },
    {
      ...
      field: 'locationType',
      align: 'center',
      format: (value) => setLocationType(value)
    },
    {
      name: 'title',
      label: '제목',
...
const setLocationType = (value) => {
  const matchedItem = filterOptions.value.locationTypeList.find((item) => item.value === value)
...
const filterSetting = async () => {
  const response = await sitemanagementApi.getBannerCodeList().then((res) => {
    const apiData = res.data
    apiData.postYnList.unshift({
      dtlCd: 'ALL',
      dtlCdNm: '전체'
    })
    apiData.locationTypeList.unshift({ dtlCd: 'ALL', dtlCdNm: '전체' })
  ...
  const inputLocationType = filterOptions.value.locationTypeList.find(
    (item) => item.value === query.get('inputLocationType')
  )
  ...
  tableModel.value.filter = {
    ...
    inputLocationType: inputLocationType ?? filterOptions.value.locationTypeList[0]
  }
...
const onRequest = async (_props) => {
  ...
  const tableRowResponse = await sitemanagementApi.getMainBannerList({
...
const goRegist = () => {
  router.push({ name: 'BannersDetail' })
}

const goDetail = (event, row) => {
  event.preventDefault()

  router.push({ name: 'BannersDetail', query: { bannerSeq: row.bannerSubSeq } })
}
</script>
<style>
tbody:empty::before {
